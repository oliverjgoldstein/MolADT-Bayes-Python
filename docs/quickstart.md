...
- `make freesolv` runs the FreeSolv 30-feature full MolADT empirical-Bayes GP benchmark.
- `make freesolv-20split` runs the same GP on 20 deterministic random splits and writes a summary plus split assignments under `results/freesolv_small_feature_gp/`.
- `make freesolv-ablation` runs the atom-bag/SMILES-adjacency-graph/full-MolADT representation ablation and writes compact CSVs plus an SVG under `results/freesolv_ablation/`.
