...
The ZINC timing benchmark is an interoperability/runtime benchmark rather than the central representation comparison. It now uses the fixed six-stage SMILES-vs-MolADT timing path: source SMILES reads, `SMILES -> JSON`, cached `SDF -> MolADT`, cached `SDF -> SMILES`, `MolADT -> JSON`, and `JSON -> MolADT`.
