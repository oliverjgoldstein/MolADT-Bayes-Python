...
MolADT's core idea is simple: keep molecule structure as typed data, then parse, validate, featurize, score, and export that same object. At the bonding layer, every edge is represented by a bonding system; single, double, triple, and quadruple covalent bonds are one-edge systems sharing 2, 4, 6, and 8 electrons and display as `single covalent`, `double covalent`, `triple covalent`, and `quadruple covalent`. Ionic contacts are also one-edge systems, but share 0 electrons, display as `ionic`, and store charge on the atoms. Pretty printers display edge rows from that bonding-system layer, including the total electrons shared over each edge; viewers list the same explicit systems.
